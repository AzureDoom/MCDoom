...
public class BaronEntity extends DemonEntity implements GeoEntity {
	...
	public void registerControllers(ControllerRegistrar controllers) {
		controllers.add(new AnimationController<>(this, "livingController", 0, event -> {
			...
			if (event.getKeyframeData().getSound().matches("attack"))
				if (this.level.isClientSide())
					this.getLevel().playLocalSound(this.getX(), this.getY(), this.getZ(), DoomSounds.BARON_AMBIENT,
							SoundSource.HOSTILE, 0.25F, 1.0F, false);
		}));
	}
		...
		this.goalSelector
				.addGoal(4,
						new RangedStrafeAttackGoal(this, new BaronEntity.FireballAttack(this)
								.setProjectileOriginOffset(0.8, 0.4, 0.8).setDamage(DoomConfig.baron_ranged_damage),
								1.2, 2));
	...
	public static AttributeSupplier.Builder createMobAttributes() {
		return LivingEntity.createLivingAttributes().add(Attributes.FOLLOW_RANGE, 40.0D)
				.add(Attributes.MAX_HEALTH, DoomConfig.baron_health).add(Attributes.ATTACK_DAMAGE, 0.0D)
