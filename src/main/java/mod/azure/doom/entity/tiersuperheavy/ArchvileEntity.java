...
public class ArchvileEntity extends DemonEntity implements SmartBrainOwner<ArchvileEntity> {
	...
	public void registerControllers(ControllerRegistrar controllers) {
		controllers.add(new AnimationController<>(this, "livingController", 0, event -> {
			...
			if (event.getKeyframeData().getSound().matches("attack"))
				if (level().isClientSide())
					level().playLocalSound(this.getX(), this.getY(), this.getZ(), DoomSounds.ARCHVILE_SCREAM.get(), SoundSource.HOSTILE, 0.25F, 1.0F, false);
	...
	public static AttributeSupplier.Builder createMobAttributes() {
		return LivingEntity.createLivingAttributes().add(Attributes.FOLLOW_RANGE, 40.0D).add(Attributes.MAX_HEALTH, DoomConfig.SERVER.archvile_health.get()).add(Attributes.ATTACK_DAMAGE, 0.0D).add(Attributes.MOVEMENT_SPEED, 0.25D).add(Attributes.ATTACK_KNOCKBACK, 0.0D);
	}
	...
	public void spawnFlames(double x, double z, double maxY, double y, float yaw, int warmup) {
		...
		if (flag) {
			final var fang = new DoomFireEntity(level(), x, blockpos.getY() + d0, z, yaw, 1, this,DoomConfig.SERVER.archvile_ranged_damage.get().floatValue());
