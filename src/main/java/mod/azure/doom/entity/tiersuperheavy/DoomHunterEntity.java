...
public class DoomHunterEntity extends DemonEntity implements SmartBrainOwner<DoomHunterEntity> {
	...
	public BrainActivityGroup<DoomHunterEntity> getFightTasks() {
		return BrainActivityGroup.fightTasks(new InvalidateAttackTarget<>().invalidateIf((target, entity) -> !target.isAlive() || !entity.hasLineOfSight(target)), new SetWalkTargetToAttackTarget<>().speedMod(1.05F), new DemonProjectileAttack<>(7).attackInterval(mob -> 80), new DemonMeleeAttack<>(8).attackInterval(mob -> 80));
	}
