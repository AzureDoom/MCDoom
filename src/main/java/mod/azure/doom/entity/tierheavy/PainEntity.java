...
public class PainEntity extends DemonEntity implements SmartBrainOwner<PainEntity> {
	...
	public static AttributeSupplier.Builder createMobAttributes() {
		return LivingEntity.createLivingAttributes().add(Attributes.FOLLOW_RANGE, 40.0D).add(Attributes.ATTACK_DAMAGE, DoomConfig.SERVER.lost_soul_melee_damage.get()).add(Attributes.KNOCKBACK_RESISTANCE, 0.6f).add(Attributes.MAX_HEALTH, DoomConfig.SERVER.pain_health.get()).add(Attributes.ATTACK_DAMAGE, 0.0D).add(Attributes.MOVEMENT_SPEED, 0.25D).add(Attributes.ATTACK_KNOCKBACK, 0.0D);
	}
	...
	protected void tickDeath() {
		...
		if (deathTime == 30) {
			...
			if (!level().isClientSide()) {
				final var lost_soul = DoomEntities.LOST_SOUL.get().create(level());
				lost_soul.moveTo(this.getX(), this.getY(), this.getZ(), 0, 0);
				level().addFreshEntity(lost_soul);
				final var lost_soul1 = DoomEntities.LOST_SOUL.get().create(level());
				lost_soul1.moveTo(this.getX(), this.getY(), this.getZ(), 0, 0);
				level().addFreshEntity(lost_soul1);
				final var lost_soul2 = DoomEntities.LOST_SOUL.get().create(level());
