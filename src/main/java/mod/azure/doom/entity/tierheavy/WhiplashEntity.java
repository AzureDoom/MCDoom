...
public class WhiplashEntity extends DemonEntity implements SmartBrainOwner<WhiplashEntity> {
    ...
    public BrainActivityGroup<WhiplashEntity> getFightTasks() {
        return BrainActivityGroup.fightTasks(new InvalidateAttackTarget<>().invalidateIf((target, entity) -> !target.isAlive()), new SetWalkTargetToAttackTarget<>().speedMod((owner, entity) -> 1.75F), new DemonMeleeAttack<>(5));
    }
