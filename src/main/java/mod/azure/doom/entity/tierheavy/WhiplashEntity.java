...
public class WhiplashEntity extends DemonEntity implements SmartBrainOwner<WhiplashEntity> {
	...
	public static AttributeSupplier.Builder createMobAttributes() {
		return LivingEntity.createLivingAttributes().add(Attributes.FOLLOW_RANGE, 40.0D).add(Attributes.KNOCKBACK_RESISTANCE, 0.6f).add(Attributes.MAX_HEALTH, DoomConfig.SERVER.whiplash_health.get()).add(Attributes.ATTACK_DAMAGE, DoomConfig.SERVER.whiplash_melee_damage.get()).add(Attributes.MOVEMENT_SPEED, 0.25D).add(Attributes.ATTACK_KNOCKBACK, 0.0D);
	}
