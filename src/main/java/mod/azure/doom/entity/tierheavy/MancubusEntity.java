...
public class MancubusEntity extends DemonEntity implements SmartBrainOwner<MancubusEntity> {
	...
	public void registerControllers(ControllerRegistrar controllers) {
		controllers.add(new AnimationController<>(this, "livingController", 0, event -> {
			...
			if (event.getKeyframeData().getSound().matches("talk"))
				if (level.isClientSide())
					getLevel().playLocalSound(this.getX(), this.getY(), this.getZ(), DoomSounds.MANCUBUS_STEP.get(), SoundSource.HOSTILE, 0.25F, 1.0F, false);
			...
			if (event.getKeyframeData().getSound().matches("attack"))
				if (level.isClientSide())
					getLevel().playLocalSound(this.getX(), this.getY(), this.getZ(), DoomSounds.ROCKET_FIRING.get(), SoundSource.HOSTILE, 0.25F, 1.0F, true);
	...
	public void spawnFlames(double x, double z, double maxY, double y, float yaw, int warmup) {
		...
		if (flag) {
			final var fang = new DoomFireEntity(level, x, blockpos.getY() + d0, z, yaw, 1, this, DoomConfig.SERVER.mancubus_ranged_damage.get().floatValue());
