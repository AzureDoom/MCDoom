...
public class BFGEntity extends AbstractArrow implements GeoEntity {
	...
	public void tick() {
		...
		this.getCommandSenderWorld().getEntities(this, aabb).forEach(e -> {
			...
			if (!(e instanceof Player || e instanceof EnderDragon || e instanceof GoreNestEntity || e instanceof IconofsinEntity || e instanceof ArchMakyrEntity || e instanceof GladiatorEntity || e instanceof MotherDemonEntity) && (e instanceof Monster || e instanceof Slime || e instanceof Phantom || e instanceof DemonEntity || e instanceof Shulker || e instanceof Hoglin || (e == listEntity))) {
				if (e.isAlive()) {
					e.hurt(damageSources().explosion(this, shooter), DoomConfig.SERVER.bfgball_damage_aoe.get().floatValue());
					this.setTargetedEntity(e.getId());
				}
			}
			if (e instanceof EnderDragon) {
				if (e.isAlive()) {
					((EnderDragon) e).head.hurt(damageSources().playerAttack((Player) this.shooter), DoomConfig.SERVER.bfgball_damage_dragon.get().floatValue() * 0.3F);
					this.setTargetedEntity(e.getId());
				}
			}
			if (e instanceof IconofsinEntity || e instanceof ArchMakyrEntity || e instanceof GladiatorEntity || e instanceof MotherDemonEntity) {
				if (e.isAlive()) {
					e.hurt(damageSources().playerAttack((Player) this.shooter), DoomConfig.SERVER.bfgball_damage_aoe.get().floatValue() * 0.1F);
				}
			}
		});
	}
	...
	public void doDamage() {
		...
		this.getCommandSenderWorld().getEntities(this, aabb).forEach(e -> {
			...
			if (e instanceof IconofsinEntity || e instanceof ArchMakyrEntity || e instanceof GladiatorEntity || e instanceof MotherDemonEntity)
				if (e.isAlive())
					e.hurt(damageSources().playerAttack((Player) this.shooter), DoomConfig.SERVER.bfgball_damage.get().floatValue() * 0.1F);
		});
	}
