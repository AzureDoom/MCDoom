	...
	protected void onHitEntity(EntityHitResult entityHitResult) {
		...
		if (!level().isClientSide()) {
			...
			if (!(entity instanceof DemonEntity))
				entity.hurt(damageSources().mobAttack((LivingEntity) entity2), (float) (DoomMod.config.gladiator_ranged_damage + (shooter.getEntityData().get(GladiatorEntity.DEATH_STATE) == 1 ? DoomMod.config.gladiator_phaseone_damage_boost : 0)));
