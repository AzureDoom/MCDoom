...
public class RangedShotgunAttackGoal<T extends CreatureEntity & IRangedAttackMob> extends Goal {
	...
	public RangedShotgunAttackGoal(DemonEntity mob, double moveSpeedAmpIn, int attackCooldownIn, float maxAttackDistanceIn, int state) {
		this.entity = mob;
	...
	protected boolean isBowInMainhand() {
		return this.entity.getMainHandItem().getItem() instanceof Shotgun
				|| this.entity.getOffhandItem().getItem() instanceof Shotgun;
	}
	...
	public void tick() {
		...
		if (livingentity != null) {
			...
			if (this.entity.isUsingItem() && this.entity.distanceTo(livingentity) >= 6.0D) {
				if (!flag && this.seeTime < -60) {
					...
					if (i >= 20) {
						this.entity.stopUsingItem();
						((IRangedAttackMob) this.entity).performRangedAttack(livingentity, Shotgun.getArrowVelocity(i));
