...
public class ImpStoneEntity extends DemonEntity implements SmartBrainOwner<ImpStoneEntity> {
	...
	public BrainActivityGroup<ImpStoneEntity> getFightTasks() {
		return BrainActivityGroup.fightTasks(new InvalidateAttackTarget<>().invalidateIf((target, entity) -> !target.isAlive()), new SetWalkTargetToAttackTarget<>().speedMod((owner, entity) -> 1.25F), new AnimatableMeleeAttack<>(0));
	}
