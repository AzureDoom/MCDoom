...
public class UnwillingEntity extends DemonEntity implements GeoEntity {
		...
		controllers.add(new AnimationController<>(this, "livingController", 0, event -> {
			...
			if (event.getKeyframeData().getSound().matches("walk"))
				if (this.level.isClientSide())
					this.getLevel().playLocalSound(this.getX(), this.getY(), this.getZ(), DoomSounds.ZOMBIEMAN_AMBIENT.get(),
	...
	public static AttributeSupplier.Builder createMobAttributes() {
		return LivingEntity.createLivingAttributes().add(Attributes.FOLLOW_RANGE, 40.0D)
				...
				.add(Attributes.ATTACK_DAMAGE, DoomConfig.SERVER.unwilling_melee_damage.get()).add(Attributes.MOVEMENT_SPEED, 0.25D)
