...
public class PossessedScientistEntity extends DemonEntity implements GeoEntity, SmartBrainOwner<PossessedScientistEntity> {
	...
	public BrainActivityGroup<PossessedScientistEntity> getCoreTasks() {
		return BrainActivityGroup.coreTasks(new LookAtTarget<>(), new LookAtTargetSink(40, 300), new StrafeTarget<>().speedMod(0.95F), new MoveToWalkTarget<>());
	}
	...
	public static AttributeSupplier.Builder createMobAttributes() {
		return LivingEntity.createLivingAttributes().add(Attributes.FOLLOW_RANGE, 40.0D).add(Attributes.MAX_HEALTH, DoomConfig.possessed_scientist_health).add(Attributes.ATTACK_DAMAGE, DoomConfig.possessed_scientist_melee_damage).add(Attributes.MOVEMENT_SPEED, 0.15D).add(Attributes.ATTACK_KNOCKBACK, 0.0D);
	}
