...
public class PossessedScientistEntity extends DemonEntity implements GeoEntity, SmartBrainOwner<PossessedScientistEntity> {
	...
	public BrainActivityGroup<PossessedScientistEntity> getFightTasks() {
		return BrainActivityGroup.fightTasks(new InvalidateAttackTarget<>().invalidateIf((target, entity) -> !target.isAlive() || !entity.hasLineOfSight(target)), new SetWalkTargetToAttackTarget<>().speedMod(0.75F), new DemonProjectileAttack<>(5).attackInterval(mob -> 80), new DemonMeleeAttack<>(5));
	}
	...
	public static AttributeSupplier.Builder createMobAttributes() {
		return LivingEntity.createLivingAttributes().add(Attributes.FOLLOW_RANGE, 40.0D).add(Attributes.MAX_HEALTH, DoomConfig.SERVER.possessed_scientist_health.get()).add(Attributes.ATTACK_DAMAGE, DoomConfig.SERVER.possessed_scientist_melee_damage.get()).add(Attributes.MOVEMENT_SPEED, 0.15D).add(Attributes.ATTACK_KNOCKBACK, 0.0D);
	}
