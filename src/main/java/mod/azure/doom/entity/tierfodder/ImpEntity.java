...
public class ImpEntity extends DemonEntity implements GeoEntity {
	...
	protected void registerGoals() {
		...
		goalSelector.addGoal(4, new RangedStrafeAttackGoal(this, new FireballAttack(this, false).setProjectileOriginOffset(0.8, 0.8, 0.8).setDamage(DoomConfig.imp_ranged_damage).setSound(SoundEvents.BLAZE_SHOOT, 1.0F, 1.4F + getRandom().nextFloat() * 0.35F), 1.2D, 5, 30, 15, 15F, 1).setMultiShot(2, 3));
	...
	public static AttributeSupplier.Builder createMobAttributes() {
		return LivingEntity.createLivingAttributes().add(Attributes.FOLLOW_RANGE, 40.0D).add(Attributes.MAX_HEALTH, DoomConfig.imp_health).add(Attributes.ATTACK_DAMAGE, 0.0D).add(Attributes.MOVEMENT_SPEED, 0.25D).add(Attributes.ATTACK_KNOCKBACK, 0.0D);
	}
