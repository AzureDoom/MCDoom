...
public class ShotgunguyEntity extends DemonEntity implements GeoEntity {
	...
	public void registerControllers(ControllerRegistrar controllers) {
		controllers.add(new AnimationController<>(this, "livingController", 0, event -> {
			...
			if (event.getKeyframeData().getSound().matches("attack"))
				if (this.level.isClientSide())
					this.getLevel().playLocalSound(this.getX(), this.getY(), this.getZ(), DoomSounds.SHOTGUN_SHOOT.get(),
							SoundSource.HOSTILE, 0.25F, 1.0F, false);
		}));
	}
		...
		this.goalSelector.addGoal(4,
				new RangedAttackGoal(this,
						new RangedAttack(this).setProjectileOriginOffset(0.8, 0.4, 0.8)
								.setDamage(DoomConfig.SERVER.shotgun_damage.get().floatValue()).setSound(DoomSounds.SHOTGUN_SHOOT.get(), 1.0F, 1.0F),
						1.1D));
	...
	public class RangedAttack extends AbstractRangedAttack {
		...
		public AttackSound getDefaultAttackSound() {
			return new AttackSound(DoomSounds.SHOTGUN_SHOOT.get(), 1, 1);
		}
	...
	public static AttributeSupplier.Builder createMobAttributes() {
		return LivingEntity.createLivingAttributes().add(Attributes.FOLLOW_RANGE, 40.0D)
				.add(Attributes.MAX_HEALTH, DoomConfig.SERVER.shotgunguy_health.get()).add(Attributes.ATTACK_DAMAGE, 2.5D)
