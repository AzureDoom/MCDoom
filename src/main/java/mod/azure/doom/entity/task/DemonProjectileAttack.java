...
//			archvileEntity.teleportRandomly();
		...
		if (entity instanceof MancubusEntity mancubusEntity) {
			if (mancubusEntity.distanceTo(target) < 8.0D && mancubusEntity.distanceTo(target) > 3.0D) // shoot flames
				mancubusEntity.shootMancubus(mancubusEntity, DoomConfig.SERVER.mancubus_ranged_damage.get().floatValue());
			else // shoot ball
				mancubusEntity.shootBaron(mancubusEntity, DoomConfig.SERVER.mancubus_ranged_damage.get().floatValue(), 0, 0, 0);
		}
		...
		if (entity instanceof MotherDemonEntity motherdemonEntity) {
			...
			if (entity.getHealth() <= entity.getMaxHealth() * 0.50) { // summon flames
				...
				entity.shootFireball(this.target, DoomConfig.SERVER.motherdemon_ranged_damage.get().floatValue() + (motherdemonEntity.getDeathState() == 1 ? DoomConfig.SERVER.motherdemon_phaseone_damage_boos.get().floatValue() : 0), 0);
				entity.shootFireball(this.target, DoomConfig.SERVER.motherdemon_ranged_damage.get().floatValue() + (motherdemonEntity.getDeathState() == 1 ? DoomConfig.SERVER.motherdemon_phaseone_damage_boos.get().floatValue() : 0), 3);
				entity.shootFireball(this.target, DoomConfig.SERVER.motherdemon_ranged_damage.get().floatValue() + (motherdemonEntity.getDeathState() == 1 ? DoomConfig.SERVER.motherdemon_phaseone_damage_boos.get().floatValue() : 0), -3);
			}
		}
		...
		if (entity instanceof DoomHunterEntity doomHunterEntity) {
			...
			if (doomHunterEntity.getDeathState() == 0) {
				doomHunterEntity.shootRocket(this.target, DoomConfig.SERVER.doomhunter_ranged_damage.get().floatValue() + (doomHunterEntity.getDeathState() == 1 ? DoomConfig.SERVER.doomhunter_extra_phase_two_damage.get().floatValue() : 0));
			}
		}
		...
		if (entity instanceof ArchMakyrEntity archMakyrEntity) {
			if (entity.getRandom().nextInt(0, 4) == 1) // shoot fireball
				entity.shootFireball(entity, DoomConfig.SERVER.archmaykr_ranged_damage.get().floatValue()
						+ (archMakyrEntity.getDeathState() == 1 ? DoomConfig.SERVER.archmaykr_phaseone_damage_boost.get().floatValue() : archMakyrEntity.getDeathState() == 2 ? DoomConfig.SERVER.archmaykr_phasetwo_damage_boost.get().floatValue() : archMakyrEntity.getDeathState() == 3 ? DoomConfig.SERVER.archmaykr_phasethree_damage_boost.get().floatValue() : archMakyrEntity.getDeathState() == 4 ? DoomConfig.SERVER.archmaykr_phasefour_damage_boost.get().floatValue() : 0), 0);
			else { // summon flames
				for (var i = 1; i < 5; ++i) {
		...
		if (entity instanceof IconofsinEntity iconEntity) {
			...
			if (randomAttack == 1) {// Summon Fire on target
				...
				entity.shootFireball(entity, DoomConfig.SERVER.icon_melee_damage.get().floatValue() + (iconEntity.getDeathState() == 1 ? DoomConfig.SERVER.icon_phaseone_damage_boos.get().floatValue() : 0), 0);
				if (entity.getHealth() < (entity.getMaxHealth() * 0.50))
