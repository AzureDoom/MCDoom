...
public class IconofsinEntity extends DemonEntity implements SmartBrainOwner<IconofsinEntity> {
		...
		controllers.add(new AnimationController<>(this, "livingController", 0, event -> {
			...
			if (event.getKeyframeData().getSound().matches("walk"))
				if (level.isClientSide())
					getLevel().playLocalSound(this.getX(), this.getY(), this.getZ(), DoomSounds.CYBERDEMON_STEP.get(), SoundSource.HOSTILE, 0.25F, 1.0F, false);
		}));
	}
	...
	public boolean doHurtTarget(Entity target) {
		...
		final var bl = target.hurt(DamageSource.mobAttack(this), DoomConfig.SERVER.icon_melee_damage.get().floatValue() + (this.getDeathState() == 1 ? DoomConfig.SERVER.icon_phaseone_damage_boos.get().floatValue() : 0));
		if (bl) {
