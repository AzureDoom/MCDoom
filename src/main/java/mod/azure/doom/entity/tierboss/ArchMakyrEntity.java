...
public class ArchMakyrEntity extends DemonEntity implements SmartBrainOwner<ArchMakyrEntity> {
    ...
    public BrainActivityGroup<ArchMakyrEntity> getFightTasks() {
        return BrainActivityGroup.fightTasks(new InvalidateAttackTarget<>().invalidateIf((target, entity) -> !target.isAlive()), new SetWalkTargetToAttackTarget<>().speedMod((owner, entity) -> 2.05F), new DemonProjectileAttack<>(7).attackInterval(mob -> 240));
    }
