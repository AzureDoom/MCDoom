	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(4,
				new RangedStrafeAttackGoal(this, new SpiderMastermind2016Entity.FireballAttack(this)
						.setProjectileOriginOffset(0.8, 0.2, 0.8).setDamage(DoomConfig.SERVER.spider_mastermind_ranged_damage.get().floatValue()),
						1.0D, 20, 30, 15, 15F, 1));
	...
	public void registerControllers(ControllerRegistrar controllers) {
		controllers.add(new AnimationController<>(this, "livingController", 0, event -> {
			...
			if (event.getKeyframeData().getSound().matches("walk"))
				if (this.level.isClientSide)
					this.getLevel().playLocalSound(this.getX(), this.getY(), this.getZ(),
							DoomSounds.SPIDERDEMON_AMBIENT.get(), SoundSource.HOSTILE, 0.25F, 1.0F, false);
			...
			if (event.getKeyframeData().getSound().matches("attack"))
				if (this.level.isClientSide)
					this.getLevel().playLocalSound(this.getX(), this.getY(), this.getZ(), DoomSounds.PLASMA_FIRING.get(),
