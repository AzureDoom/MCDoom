...
public class TentacleEntity extends DemonEntity implements IAnimatable, IAnimationTickable {
		...
		public void tick() {
			...
			if (livingentity != null) {
				...
				if (this.entity.canSee(livingentity)) {
					++this.cooldown;
					if (this.entity.getEntityWorld().getOtherEntities(this.entity, aabb2).contains(livingentity)) {
						if (this.cooldown == 2) {
	...
	public static DefaultAttributeContainer.Builder createMobAttributes() {
		return LivingEntity.createLivingAttributes().add(EntityAttributes.GENERIC_FOLLOW_RANGE, 25.0D)
				.add(EntityAttributes.GENERIC_KNOCKBACK_RESISTANCE, 1.0f).add(EntityAttributes.GENERIC_MAX_HEALTH, 5.0D)
