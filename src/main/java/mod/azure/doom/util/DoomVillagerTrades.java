...
public class DoomVillagerTrades {
	...
	public static void addTrades() {
		if (DoomMod.config.misc.enable_weaponsmith_trades) {
			...
			weapon_trades
					.add(new TradeOffers.ProcessItemFactory(Items.EMERALD, 1, 1, DoomItems.ARGENT_ENERGY, 6, 12, 5));
			weapon_trades.add(new TradeOffers.ProcessItemFactory(Items.EMERALD, 2, DoomItems.BULLETS, 6, 12, 5));
			weapon_trades
					.add(new TradeOffers.ProcessItemFactory(Items.EMERALD, 3, DoomItems.CHAINGUN_BULLETS, 6, 12, 5));
		...
		TradeOffers.PROFESSION_TO_LEVELED_TRADE.get(VillagerProfession.TOOLSMITH).put(3, tool_trades.toArray(result));
		}
		...
		TradeOffers.PROFESSION_TO_LEVELED_TRADE.get(VillagerProfession.MASON).put(3, block_trades.toArray(result));
		}
	}
}