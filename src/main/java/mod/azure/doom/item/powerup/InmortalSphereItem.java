...
public class InmortalSphereItem extends Item {
		...
		if (livingEntityIn instanceof ServerPlayer) {
			...
			if (!worldIn.isClientSide) {
				...
				if (ModList.get().isLoaded("pmmo")) {
					PMMOCompat.awardMagicXp(playerentity, DoomConfig.SERVER.inmortal_consume_xp_pmmo.get());
				}
			}
		}
	}
	...
	public void appendHoverText(ItemStack stack, Level worldIn, List<Component> tooltip, TooltipFlag flagIn) {
		tooltip.add(new TranslatableComponent("doom.inmortal.text").withStyle(ChatFormatting.ITALIC));
