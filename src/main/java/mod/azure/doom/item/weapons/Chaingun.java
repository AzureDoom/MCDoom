...
public class Chaingun extends DoomBaseItem {
                    ...
                    if (!worldIn.isClientSide) {
                        ...
                        if (result != null) {
                            if (result.getEntity() instanceof LivingEntity livingEntity)
                                livingEntity.hurt(playerentity.damageSources().playerAttack(playerentity), DoomMod.config.chaingun_bullet_damage + enchantlevel * 2.0F);
                        } else {
    ...
    public void inventoryTick(ItemStack stack, Level world, Entity entity, int slot, boolean selected) {
        if (world.isClientSide && entity instanceof Player player && player.getMainHandItem().getItem() instanceof Chaingun)
    ...
    public ChaingunBulletEntity createArrow(Level worldIn, ItemStack stack, LivingEntity shooter) {
        ...
        final ChaingunBulletEntity arrowentity = new ChaingunBulletEntity(worldIn, shooter, DoomMod.config.chaingun_bullet_damage + j * 2.0F);
        return arrowentity;
    }
