				...
				if (!worldIn.isClientSide) {
					...
					if (result != null) {
						if (result.getEntity()instanceof LivingEntity livingEntity)
							livingEntity.hurt(DamageSource.playerAttack(playerentity), DoomConfig.SERVER.bullet_damage.get().floatValue() + enchantlevel * 2.0F);
					} else {
		...
		if (world.isClientSide)
			if (stack.getItem() instanceof PistolItem)
				while (Keybindings.RELOAD.consumeClick() && selected)
					DoomPacketHandler.PISTOL.sendToServer(new PistolLoadingPacket(slot));
	}
