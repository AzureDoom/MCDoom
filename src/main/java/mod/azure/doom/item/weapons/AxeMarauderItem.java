...
public class AxeMarauderItem extends SwordItem {
	...
	public AxeMarauderItem() {
		super(DoomTier.DOOM_HIGHTEIR, -5, -2.5f, new Item.Properties().stacksTo(1).durability(DoomMod.config.marauder_max_uses).tab(DoomMod.DoomWeaponItemGroup));
	}
	...
	private void doDamage(LivingEntity user, final Entity target) {
		if (target instanceof LivingEntity) {
			...
			target.hurt(DamageSource.playerAttack((Player) user), target instanceof ArchMakyrEntity || target instanceof GladiatorEntity || target instanceof IconofsinEntity || target instanceof MotherDemonEntity || target instanceof SpiderMastermind2016Entity || target instanceof SpiderMastermindEntity ? DoomMod.config.marauder_axe_item_damage / 10F : DoomMod.config.marauder_axe_item_damage);
		}
	}
