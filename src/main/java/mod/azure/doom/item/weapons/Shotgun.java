...
public class Shotgun extends DoomBaseItem {
			...
			if (stack.getDamageValue() < (stack.getMaxDamage() - 1)) {
				...
				if (!worldIn.isClientSide) {
					...
					worldIn.playSound((Player) null, playerentity.getX(), playerentity.getY(), playerentity.getZ(),
							DoomSounds.SHOTGUN_SHOOT, SoundSource.PLAYERS, 1.0F, 1.5F);
		...
		if (user.getItemInHand(hand).getItem() instanceof Shotgun) {
			while (!user.isCreative() && user.getItemInHand(hand).getDamageValue() != 0
				...
				user.getCommandSenderWorld().playSound((Player) null, user.getX(), user.getY(), user.getZ(),
						DoomSounds.SHOTGUNRELOAD, SoundSource.PLAYERS, 1.00F, 1.0F);
			}
		}
	}
	...
	public void inventoryTick(ItemStack stack, Level world, Entity entity, int slot, boolean selected) {
		if (world.isClientSide()) {
			if (((Player) entity).getMainHandItem().getItem() instanceof Shotgun && ClientInit.reload.consumeClick()
					&& selected) {
