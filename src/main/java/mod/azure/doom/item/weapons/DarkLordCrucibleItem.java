...
public class DarkLordCrucibleItem extends SwordItem implements GeoItem {
	...
	public DarkLordCrucibleItem() {
		super(DoomTier.DOOM_HIGHTEIR, 1, -2.5f, new Item.Properties().stacksTo(1).durability(DoomMod.config.darkcrucible_max_uses).tab(DoomMod.DoomWeaponItemGroup));
	...
	private void doDamage(LivingEntity user, Entity target) {
		if (target instanceof LivingEntity) {
			...
			target.hurt(DamageSource.playerAttack((Player) user), target instanceof ArchMakyrEntity || target instanceof GladiatorEntity || target instanceof IconofsinEntity || target instanceof MotherDemonEntity || target instanceof SpiderMastermind2016Entity || target instanceof SpiderMastermindEntity ? DoomMod.config.darkcrucible_damage / 10F : DoomMod.config.darkcrucible_damage);
		}
	}
