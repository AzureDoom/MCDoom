...
public class MotherDemonStructure extends StructureFeature<DefaultFeatureConfig> {
	...
	private boolean isNear(ChunkGenerator chunkGenerator, long l, ChunkRandom chunkRandom, int i, int j) {
		StructureConfig structureConfig = chunkGenerator.getStructuresConfig().getForType(DoomStructures.TITAN_SKULL);
		if (structureConfig == null) {
			return false;
		} else {
			for (int k = i - 10; k <= i + 10; ++k) {
				for (int m = j - 10; m <= j + 10; ++m) {
					ChunkPos chunkPos = DoomStructures.TITAN_SKULL.getStartChunk(structureConfig, l, chunkRandom, k, m);
