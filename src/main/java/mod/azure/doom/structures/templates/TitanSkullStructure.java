...
public class TitanSkullStructure extends StructureFeature<StructurePoolFeatureConfig> {
	...
	public TitanSkullStructure(Codec<StructurePoolFeatureConfig> codec) {
		super(codec, (context) -> {
			if (!TitanSkullStructure.canGenerate(context)) {
...
//		BlockPos blockpos = context.chunkPos().getCenterAtY(33);
		BlockPos.Mutable blockpos = new BlockPos.Mutable(context.chunkPos().getStartX(), 33,
				context.chunkPos().getStartZ());
		((StructurePoolFeatureConfigAccessor) context.config()).setStructures(() -> context.registryManager()
				.get(Registry.STRUCTURE_POOL_KEY).get(new Identifier(DoomMod.MODID, "titan_skull/start_pool")));
