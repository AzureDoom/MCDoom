...
public class NetherPortalStructure extends StructureFeature<DefaultFeatureConfig> {

	public NetherPortalStructure(Codec<DefaultFeatureConfig> codec) {
		super(codec);
	}

	@Override
	public StructureStartFactory<DefaultFeatureConfig> getStructureStartFactory() {
		return NetherPortalStructure.Start::new;
	}
	...
	private boolean isNearby(ChunkGenerator generator, long worldSeed, ChunkRandom random, ChunkPos pos) {
		StructureConfig structureConfig = generator.getStructuresConfig().getForType(DoomStructures.MOTHERDEMON);
		if (structureConfig == null) {
			...
			for (int k = i - 10; k <= i + 10; ++k) {
				for (int l = j - 10; l <= j + 10; ++l) {
					ChunkPos chunkPos = DoomStructures.MOTHERDEMON.getStartChunk(structureConfig, worldSeed, random, k,
							l);
