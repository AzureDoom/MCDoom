...
public class SwordCrucibleItem extends BaseSwordItem implements GeoItem {
    ...
    public void inventoryTick(ItemStack stack, Level level, Entity entity, int slot, boolean selected) {
        ...
        if (!level.isClientSide()) if (playerentity.getMainHandItem().is(this) && selected) {
    ...
    public boolean hurtEnemy(ItemStack stack, LivingEntity target, LivingEntity miner) {
        if (miner instanceof Player playerentity && stack.getDamageValue() < stack.getMaxDamage() - 1 && playerentity.getMainHandItem().getItem() instanceof SwordCrucibleItem) {
    ...
    public void createRenderer(Consumer<Object> consumer) {
        consumer.accept(new RenderProvider() {
            private final SwordCrucibleRender renderer = null;

            @Override
            public BlockEntityWithoutLevelRenderer getCustomRenderer() {
                if (renderer == null) return new SwordCrucibleRender();
