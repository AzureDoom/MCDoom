...
public class DemonProjectileAttack<E extends DemonEntity> extends CustomDelayedRangedBehaviour<E> {
    ...
    protected void doDelayedAction(E entity) {
        ...
        if (entity instanceof MaykrDroneEntity) entity.shootBolt(this.target, damage);

        if (entity instanceof BloodMaykrEntity) entity.shootBloodBolt(this.target, damage);
        ...
        if (entity instanceof CyberdemonEntity) entity.shootRocket(this.target, damage);
        ...
        if (entity instanceof SummonerEntity summonerEntity) {
            if (summonerEntity.getRandom().nextInt(0, 40) >= 17) // spawn flames
                for (var j = 0; j < 16; ++j)
                    summonerEntity.spawnFlames(entity.getX() + Mth.cos((float) Mth.atan2(this.target.getZ() - entity.getZ(), this.target.getX() - summonerEntity.getX())) * (1.25D * (j + 1)), entity.getZ() + Mth.sin((float) Mth.atan2(this.target.getZ() - entity.getZ(), this.target.getX() - summonerEntity.getX())) * (1.25D * (j + 1)), Math.min(this.target.getY(), summonerEntity.getY()), Math.max(this.target.getY(), summonerEntity.getY()) + 1.0D, (float) Mth.atan2(this.target.getZ() - entity.getZ(), this.target.getX() - summonerEntity.getX()), 32);
            else // spawn wave
    ...
    private void hitScanAttack(LivingEntity livingEntity) {
        ...
        final var checkBlocking = TargetingConditions.forCombat().range(16).selector(target1 -> !target1.getUseItem().is(Items.SHIELD));
        livingEntity.level().getNearbyEntities(LivingEntity.class, checkBlocking, livingEntity, aabb).stream().findFirst().ifPresent(target2 -> {
            BehaviorUtils.lookAtEntity(livingEntity, target2);
            target2.hurt(livingEntity.damageSources().mobAttack(livingEntity), livingEntity instanceof ChaingunnerEntity ? MCDoom.config.chaingun_bullet_damage : damage);
        });
    }
    ...
    private void marauderAttacks(MarauderEntity marauderEntity) {
        ...
        final var checkBlocking = TargetingConditions.forCombat().range(16).selector(target1 -> !target1.getUseItem().is(Items.SHIELD));
        marauderEntity.level().getNearbyEntities(LivingEntity.class, checkBlocking, marauderEntity, aabb).stream().findFirst().ifPresent(target2 -> {
    ...
    private void archvileAttacks(ArchvileEntity archvileEntity) {
        archvileEntity.getCommandSenderWorld().getEntities(archvileEntity, new AABB(archvileEntity.blockPosition().above()).inflate(24D, 24D, 24D)).forEach(e -> {
        ...
        if (archvileEntity.distanceTo(target) < 8.0D) { // shoot flames
            for (var j = 0; j < 15; ++j) {
                ...
                archvileEntity.spawnFlames(archvileEntity.getX() + Mth.cos(h) * 1.5D, archvileEntity.getZ() + Mth.sin(h) * 1.5D, d, e, h, 0);
            }

            for (var j = 0; j < 18; ++j) {
                float h = f + j * 3.1415927F * 2.0F / 8.0F + 1.2566371F;
                archvileEntity.spawnFlames(archvileEntity.getX() + Mth.cos(h) * 2.5D, archvileEntity.getZ() + Mth.sin(h) * 2.5D, d, e, h, 3);
            }
        } else // shoot ball
            for (var j = 0; j < 26; ++j) {
                final double l1 = 1.25D * (j + 1);
                archvileEntity.spawnFlames(archvileEntity.getX() + Mth.cos(f) * l1, archvileEntity.getZ() + Mth.sin(f) * l1, d, e, f, 32);
            }
    }
    ...
    private void motherDemonAttacks(MotherDemonEntity motherdemonEntity) {
        ...
        if (motherdemonEntity.getHealth() <= motherdemonEntity.getMaxHealth() * 0.50) { // summon flames
            for (var l = 0; l < 32; ++l) {
                final var f1 = (float) Mth.atan2(this.target.getZ() - motherdemonEntity.getZ(), this.target.getX() - motherdemonEntity.getX()) + l * (float) Math.PI * 0.4F;
                for (var y = 0; y < 5; ++y)
                    motherdemonEntity.spawnFlames(this.target.getX() + Mth.cos(f1) * this.target.getRandom().nextDouble() * 1.5D, this.target.getZ() + Mth.sin(f1) * this.target.getRandom().nextDouble() * 1.5D, Math.min(this.target.getY(), this.target.getY()), Math.max(this.target.getY(), this.target.getY()) + 1.0D, f1, 0);
            }
            this.target.setDeltaMovement(this.target.getDeltaMovement().multiply(0.4f, 1.4f, 0.4f));
        } else { // shoot fireballs
            motherdemonEntity.shootFireball(this.target, MCDoom.config.motherdemon_ranged_damage + this.extraMotherDemonDamage(motherdemonEntity), 0);
            motherdemonEntity.shootFireball(this.target, MCDoom.config.motherdemon_ranged_damage + this.extraMotherDemonDamage(motherdemonEntity), 3);
            motherdemonEntity.shootFireball(this.target, MCDoom.config.motherdemon_ranged_damage + this.extraMotherDemonDamage(motherdemonEntity), -3);
        }
    }
    ...
    private void doomHunterAttacks(DoomHunterEntity doomHunterEntity) {
        if (doomHunterEntity.getDeathState() == 1) {
            for (int l = 0; l < 16; ++l)
                doomHunterEntity.spawnFlames(doomHunterEntity.getX() + Mth.cos((float) Mth.atan2(this.target.getZ() - doomHunterEntity.getZ(), this.target.getX() - doomHunterEntity.getX())) * 1.25D * (l + 1), doomHunterEntity.getZ() + Mth.sin((float) Mth.atan2(this.target.getZ() - doomHunterEntity.getZ(), this.target.getX() - doomHunterEntity.getX())) * 1.25D * (l + 1), Math.min(this.target.getY(), this.target.getY()), Math.max(this.target.getY(), this.target.getY()) + 1.0D, (float) Mth.atan2(this.target.getZ() - doomHunterEntity.getZ(), this.target.getX() - doomHunterEntity.getX()), l);
        }
    ...
    private void archMakyrAttacks(ArchMakyrEntity archMakyrEntity) {
        if (archMakyrEntity.getRandom().nextInt(0, 4) == 1) // shoot fireball
            archMakyrEntity.shootFireball(archMakyrEntity, MCDoom.config.archmaykr_ranged_damage + this.extraArchMaykerDamage(archMakyrEntity), 0);
        else { // summon flames
            for (var i = 1; i < 5; ++i) {
                var f1 = (float) Mth.atan2(this.target.getZ() - archMakyrEntity.getZ(), this.target.getX() - archMakyrEntity.getX()) + i * (float) Math.PI * 0.4F;
                for (var y = 0; y < 5; ++y)
                    archMakyrEntity.spawnFlames(this.target.getX() + Mth.cos(f1) * this.target.getRandom().nextDouble() * 1.5D, this.target.getZ() + Mth.sin(f1) * this.target.getRandom().nextDouble() * 1.5D, Math.min(this.target.getY(), this.target.getY()), Math.max(this.target.getY(), this.target.getY()) + 1.0D, f1, 0);
            }
            this.target.setDeltaMovement(this.target.getDeltaMovement().multiply(0.4f, 1.4f, 0.4f));
        }
    }
    ...
    private void iconAttacks(IconofsinEntity iconEntity) {
        ...
        if (randomAttack == 1) {// Summon Fire on target
            for (var i = 1; i < 5; ++i) {
                var f1 = (float) Mth.atan2(this.target.getZ() - iconEntity.getZ(), this.target.getX() - iconEntity.getX()) + i * (float) Math.PI * 0.4F;
                for (var y = 0; y < 5; ++y)
                    iconEntity.spawnFlames(this.target.getX() + Mth.cos(f1) * this.target.getRandom().nextDouble() * 1.5D, this.target.getZ() + Mth.sin(f1) * this.target.getRandom().nextDouble() * 1.5D, Math.min(this.target.getY(), this.target.getY()), Math.max(this.target.getY(), this.target.getY()) + 1.0D, f1, 0);
            }
            ...
            iconEntity.shootFireball(iconEntity, MCDoom.config.icon_melee_damage + (iconEntity.getDeathState() == 1 ? MCDoom.config.icon_phaseone_damage_boos : 0), 0);
            if (iconEntity.getHealth() < (iconEntity.getMaxHealth() * 0.50))
    ...
    private void gladiatorAttacks(GladiatorEntity gladiatorEntity) {
        ...
        } else gladiatorEntity.shootMace(this.target);
    }
