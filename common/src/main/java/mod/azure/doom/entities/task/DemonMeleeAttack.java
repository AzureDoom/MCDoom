...
public class DemonMeleeAttack<E extends DemonEntity> extends CustomDelayedMeleeBehaviour<E> {
    ...
    protected void doDelayedAction(E entity) {
        ...
        if (entity instanceof IconofsinEntity iconEntity) { // damage and explode the world
            ...
            for (var j = 0; j < 5; ++j) {
                float h2 = (float) Mth.atan2(target.getZ() - entity.getZ(), target.getX() - entity.getX()) + j * (float) Math.PI * 0.4F;
                mancubusEntity.spawnFlames(entity.getX() + Mth.cos(h2) * 1.5D, entity.getZ() + Mth.sin(h2) * 1.5D, Math.min(target.getY(), entity.getY()), Math.max(target.getY(), entity.getY()) + 1.0D, h2, 0);
            }
            ...
            final var checkBlocking = TargetingConditions.forCombat().range(3.0D).selector(target2 -> !target2.getUseItem().is(Items.SHIELD));
            marauderEntity.level().getNearbyEntities(LivingEntity.class, checkBlocking, marauderEntity, aabb).forEach(target1 -> target1.hurt(marauderEntity.damageSources().mobAttack(marauderEntity), (float) marauderEntity.getAttributeValue(Attributes.ATTACK_DAMAGE)));
